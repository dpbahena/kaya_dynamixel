...
DFRobot_BMI160::DFRobot_BMI160(uint8_t i2c_addr)
{
  this->i2c_addr = i2c_addr;
  Obmi160 =  new bmi160Dev; //(struct bmi160Dev *)malloc(sizeof(struct bmi160Dev));
  //Obmi160->id = BMI160_I2C_ADDR;
  //Obmi160->interface = BMI160_I2C_INTF; 
  Oaccel=  new bmi160SensorData;
  Ogyro = new bmi160SensorData;
  setI2c();
}


void DFRobot_BMI160::setI2c(){
  i2c_device_name = "/dev/i2c-1";
  i2c_driver = new I2C_Driver(i2c_device_name);
  bool openSuccess = i2c_driver->open_i2c_device();
  if(!openSuccess){
    printf("FAILED to opes I2C driver");
    assert(openSuccess == true);
  }
  
    
  // int result = ioctl(i2c_driver->get_file_descriptor(),I2C_SLAVE, i2c_addr);
  // if(result < 0){
  //   printf("Failed to init device");
  //   assert(result >= 0);
  // }

  // printf("I2C Device file descriptor: %d , state: %d\n",i2c_driver->get_file_descriptor(), i2c_driver->get_state());  
...
int8_t DFRobot_BMI160::softReset(struct bmi160Dev *dev)
{
  ...
  std::this_thread::sleep_for(milliseconds(BMI160_SOFT_RESET_DELAY_MS));
  if (rslt == BMI160_OK){
...
int8_t DFRobot_BMI160::setAccelConf(struct bmi160Dev *dev)
{
  ...
  if (rslt == BMI160_OK) {
    ...
    if (rslt == BMI160_OK) {
      ...
      std::this_thread::sleep_for(milliseconds(BMI160_ONE_MS_DELAY));
    ...
    if (dev->accelCfg.power != dev->prevAccelCfg.power) {
      ...
      if (rslt == BMI160_OK) {
        ...
        if (dev->prevAccelCfg.power == BMI160_ACCEL_SUSPEND_MODE){
          std::this_thread::sleep_for(milliseconds(BMI160_ACCEL_DELAY_MS));
        }
  ...
  if ((dev->gyroCfg.power == BMI160_GYRO_SUSPEND_MODE) || (dev->gyroCfg.power == BMI160_GYRO_NORMAL_MODE)
    ...
    if (dev->gyroCfg.power != dev->prevGyroCfg.power) {
      ...
      if (dev->prevGyroCfg.power == BMI160_GYRO_SUSPEND_MODE) {
        /* Delay of 81 ms */
        std::this_thread::sleep_for(milliseconds(BMI160_GYRO_DELAY_MS));
...
int8_t DFRobot_BMI160::I2cGetRegs(struct bmi160Dev *dev, uint8_t reg_addr, uint8_t *data, uint16_t len)
{
  ...
  //   std::this_thread::sleep_for(milliseconds(1));
  // }
  //uint8_t buffer[]={reg_addr};
  bool success = i2c_driver->write_data_then_read_data(dev->id,len, &reg_addr,len, data);
  if (!success){
...
int8_t DFRobot_BMI160::I2cSetRegs(struct bmi160Dev *dev, uint8_t reg_addr, uint8_t *data, uint16_t len)
{
  ...
  if ((dev->prevAccelCfg.power == BMI160_ACCEL_NORMAL_MODE)||(dev->prevGyroCfg.power == BMI160_GYRO_NORMAL_MODE)){
    ...
    bool success = i2c_driver->write_data(dev->id, len + 1, buffer);
    if (!success){
    ...
    for(int i = 0; i < len; i++){
      ...
      bool success = i2c_driver->write_data(dev->id, 2, buffer);
      if (!success){
        printf("Failed to write data\n");
        assert(success == true);
      }
...
int8_t DFRobot_BMI160::SPIGetRegs(struct bmi160Dev *dev, uint8_t reg_addr, uint8_t *data, uint16_t len)
{
  ...
  (void)data;
  (void)len;
  //digitalWrite(10,LOW);  // uncomment if working if SPI
  //SPI.transfer(0x6B);    // THESE 2 WERE ALREADY COMMENTED
  //SPI.transfer(0x10);
  //uint8_t buff[10]={0};
  for(int i = 0; i < 10; i++){
    //buff[i] = SPI.transfer(0x00);  // uncomment if working with SPI
  }
